...
export function EditorHelpDialog({ isOpen, onClose }: EditorHelpDialogProps) {
  ...
  // Reset to page 1 each time the dialog opens
  useEffect(() => {
    if (isOpen) setPage(1)
  }, [isOpen])

  // Focus primary button whenever page changes or dialog opens
  useEffect(() => {
    if (!isOpen) return
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">
                {page === 1 ? t('editorHelp.subtitle') : t('editorHelp.subtitlePage2')}
              </p>
            </div>
          </div>
        </div>

        {/* Page 1 — Text selection */}
        {page === 1 && (
          <>
        ...
        {page === 2 && (
          <div className="px-6 py-5 space-y-4">
            ...
            {/* Main card */}
            <div className="rounded-xl border border-orange-100 bg-orange-50 p-5 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-slate-800">{t('editorHelp.drawRegionTitle')}</h3>
                <span className="ml-auto px-2 py-0.5 rounded-full bg-orange-100 text-orange-700 text-xs font-medium">
                  {t('editorHelp.drawRegionBadge')}
                </span>
              </div>
              <p className="text-sm text-slate-600 leading-relaxed">{t('editorHelp.drawRegionDesc')}</p>
            </div>
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center gap-3">
          ...
          {/* Back button (page 2 only) */}
          {page > 1 ? (
            <button
              className="px-4 py-2.5 text-sm font-medium rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-100 transition-all duration-150"
